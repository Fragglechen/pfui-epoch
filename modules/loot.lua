pfUI:RegisterModule("loot", "vanilla:tbc:wotlk", function ()
  ...
  if C.loot.advancedloot == "1" then
    ...
    hooksecurefunc("UnitPopup_HideButtons",function()
      ...
      local unit = dropdownFrame.unit
      local name = dropdownFrame.name
      if not dropdownFrame.which or not UnitPopupMenus[dropdownFrame.which] then return end
  ...
  function pfUI.loot:CreateSlot(id)
    ...
    frame:SetScript("OnClick", function(self)
      if IsControlKeyDown() then
        DressUpItemLink(GetLootSlotLink(self:GetID()))
      elseif IsShiftKeyDown() then
        if ChatEdit_InsertLink then
          ChatEdit_InsertLink(GetLootSlotLink(self:GetID()))
        elseif ChatFrameEditBox:IsVisible() then
          ChatFrameEditBox:Insert(GetLootSlotLink(self:GetID()))
        end
      end

      StaticPopup_Hide("CONFIRM_LOOT_DISTRIBUTION")
      local numItems = GetNumLootItems()
      pfUI.loot.selectedLootButton = self:GetName()
      pfUI.loot.selectedSlot = self:GetID()
      pfUI.loot.selectedQuality = self.quality
      pfUI.loot.selectedItemName = self.name:GetText()
    ...
    frame:SetScript("OnEnter", function(self)
      if ( LootSlotIsItem(self:GetID()) ) then
