pfUI:RegisterModule("chat", "vanilla:tbc:wotlk", function ()
  ...
  UnitPopupButtons["IGNORE_PLAYER"] = { text = IGNORE_PLAYER or "Ignore Player", dist = 0 }

  local function HasUnitPopupEntry(menu, entry)
  ...
  local function AddIgnoreMenu(menuName)
    local menu = UnitPopupMenus and UnitPopupMenus[menuName]
    if not menu or HasUnitPopupEntry(menu, "IGNORE_PLAYER") then return end

    for index, value in ipairs(menu) do
      if value == "GUILD_LEAVE" or value == "RAID_TARGET_ICON" or value == "CANCEL" then
        table.insert(menu, index, "IGNORE_PLAYER")
        return
      end
    end

    table.insert(menu, "IGNORE_PLAYER")
  end

  AddIgnoreMenu("FRIEND")
  AddIgnoreMenu("PLAYER")
  AddIgnoreMenu("PARTY")
  AddIgnoreMenu("RAID")

  hooksecurefunc("UnitPopup_OnClick", function(self)
    local button = self or this
    if button and button.value == "IGNORE_PLAYER" then
      local dropdown = UIDROPDOWNMENU_INIT_MENU or UIDROPDOWNMENU_OPEN_MENU
      if type(dropdown) == "string" then dropdown = _G[dropdown] end

      local name = dropdown and dropdown.name
      if (not name or name == "") and dropdown and dropdown.unit then
  ...
  function pfUI.chat:RefreshChat()
    ...
    for i=1, NUM_CHAT_WINDOWS do
      ...
      if i == 1 then
        ...
        frame:SetWidth(pfUI.chat.left:GetWidth() - 2*default_border)
        frame:SetHeight(pfUI.chat.left:GetHeight() - panelheight - bottompadding)
        frame:Show()
      elseif i == 3 and C.chat.right.enable == "1" then
        ...
        frame:SetWidth(pfUI.chat.right:GetWidth() - 2*default_border)
        frame:SetHeight(pfUI.chat.right:GetHeight() - panelheight - bottompadding)
        frame:Show()
      ...
      elseif frame.isDocked then
        ...
        if i == 2 then
          ...
          frame:SetWidth(pfUI.chat.left:GetWidth() - 2*default_border)
          frame:SetHeight(pfUI.chat.left:GetHeight() - panelheight - bottompadding - combatlogpanel)
        else
          frame:SetPoint("TOPLEFT", pfUI.chat.left ,"TOPLEFT", default_border, -panelheight)
          frame:SetWidth(pfUI.chat.left:GetWidth() - 2*default_border)
          frame:SetHeight(pfUI.chat.left:GetHeight() - panelheight - bottompadding)
      ...
      if FCF_SetChatWindowFontSize then
        FCF_SetChatWindowFontSize(frame, tonumber(C.global.font_size) or 12)
      end
  ...
  function pfUI.chat.SetupPositions()
    ...
    FCF_SetTabPosition(ChatFrame1, 0)
  ...
  local function ChatSetupKey()
    local realm = GetRealmName() or "UnknownRealm"
    local player = UnitName("player") or "UnknownPlayer"
    return realm, player
  end

  local function HasStoredChannelSetup()
    pfUI_cache = pfUI_cache or {}
    pfUI_cache.chatsetup = pfUI_cache.chatsetup or {}

    local realm, player = ChatSetupKey()
    pfUI_cache.chatsetup[realm] = pfUI_cache.chatsetup[realm] or {}
    return pfUI_cache.chatsetup[realm][player] == true
  end

  local function MarkChannelSetupStored()
    pfUI_cache = pfUI_cache or {}
    pfUI_cache.chatsetup = pfUI_cache.chatsetup or {}

    local realm, player = ChatSetupKey()
    pfUI_cache.chatsetup[realm] = pfUI_cache.chatsetup[realm] or {}
    pfUI_cache.chatsetup[realm][player] = true
  end
  ...
  pfChatArrangeFrame:SetScript("OnEvent", function()
    if event == "PLAYER_ENTERING_WORLD" then
      ...
      if not HasStoredChannelSetup() then
        pfUI.chat.SetupChannels()
        MarkChannelSetupStored()
      else
        pfUI.chat:RefreshChat()
      end
    else
  ...
  local ScanWhoName = function() return end

  if pfUI.client <= 11200 and type(_G.FriendsFrame_OnEvent) == "function" then
  ...
  if C.chat.text.playerlinks == "1" and pfUI.client <= 11200 then
