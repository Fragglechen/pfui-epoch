pfUI:RegisterModule("buff", "vanilla:tbc:wotlk", function ()
  -- Hide Blizz
  BuffFrame:Hide()
  ...
  local function LooksLikeTexture(value)
    return type(value) == "string" and (strfind(value, "\\") or strfind(value, "/"))
  end

  local function NormalizeAuraCount(value)
    value = tonumber(value) or 0
    if value < 0 or value > 40 or value ~= floor(value) then
      return 0
    end
    return value
  end

  local function NormalizeAuraTime(duration, expires)
    local now = GetTime()
    duration = type(duration) == "number" and duration or nil
    expires = type(expires) == "number" and expires or nil

    if expires and expires > now then
      return duration, math.max(expires - now, 0)
    end

    -- Some 3.3.5-derived clients return remaining time where Blizzard
    -- normally returns an absolute expiration timestamp.
    if expires and expires > 0 and duration and duration >= expires then
  ...
  local function GetAuraInfoByIndex(index, btype)
    index = tonumber(index)
    if not index or index < 1 then return nil end

    if pfUI.client >= 20000 then
      if btype == "HELPFUL" then
        local a, b, c, d, e, f, g = UnitBuff("player", index)
        local texture = LooksLikeTexture(c) and c or LooksLikeTexture(a) and a or LooksLikeTexture(b) and b or nil
        local count = NormalizeAuraCount(type(d) == "number" and d or type(c) == "number" and c or 0)
        local dtype = (e == "Magic" or e == "Poison" or e == "Curse" or e == "Disease") and e
          or (d == "Magic" or d == "Poison" or d == "Curse" or d == "Disease") and d
          or nil
        local duration, timeleft = NormalizeAuraTime(f, g)
        if not timeleft then
          duration, timeleft = NormalizeAuraTime(e, f)
        end
        return texture, count, dtype, nil, duration, timeleft
      else
        local a, b, c, d, e, f, g = UnitDebuff("player", index)
        local texture = LooksLikeTexture(c) and c or LooksLikeTexture(a) and a or LooksLikeTexture(b) and b or nil
        local count = NormalizeAuraCount(type(d) == "number" and d or type(c) == "number" and c or 0)
        local dtype = (e == "Magic" or e == "Poison" or e == "Curse" or e == "Disease") and e
          or (d == "Magic" or d == "Poison" or d == "Curse" or d == "Disease") and d
    ...
    local bid = GetPlayerBuff(PLAYER_BUFF_START_ID + index, btype)
    if not bid or bid < 0 then return nil end

    return GetPlayerBuffTexture(bid), GetPlayerBuffApplications(bid, btype), GetPlayerBuffDispelType(bid), nil, nil, GetPlayerBuffTimeLeft(bid, btype)
  end
  ...
  local function RefreshBuffButton(buff)
    ...
    buff.texture:SetVertexColor(1,1,1,1)
    buff.texture:SetAlpha(1)
    buff.texture:Show()
    ...
    if buff.btype == "HELPFUL" and ((C.buffs.separateweapons == "0" and buff.gid <= pfUI.buff.wepbuffs.count) or (pfUI.buff.wepbuffs.count > 0 and buff.weapon ~= nil)) then
      ...
      local auraTexture, auraStacks, auraDispelType
      local maxSlots = buff.btype == "HELPFUL" and 32 or 16

      if buff.id >= 1 and buff.id <= maxSlots then
        auraTexture, auraStacks, auraDispelType = GetAuraInfoByIndex(buff.id, buff.btype)
      end

      if auraTexture and (( buff.btype == "HARMFUL" and C.buffs.debuffs == "1" ) or ( buff.btype == "HELPFUL" and C.buffs.buffs == "1" )) then
        buff.mode = buff.btype
        buff.texture:SetTexture(auraTexture)

        if buff.btype == "HARMFUL" then
          if auraDispelType == "Magic" then
            buff.backdrop:SetBackdropBorderColor(0,1,1,1)
          elseif auraDispelType == "Poison" then
            buff.backdrop:SetBackdropBorderColor(0,1,0,1)
          elseif auraDispelType == "Curse" then
            buff.backdrop:SetBackdropBorderColor(1,0,1,1)
          elseif auraDispelType == "Disease" then
      ...
      elseif pfUI.client < 20000 and GetPlayerBuffTexture(buff.bid) and (( buff.btype == "HARMFUL" and C.buffs.debuffs == "1" ) or ( buff.btype == "HELPFUL" and C.buffs.buffs == "1" )) then
      ...
      local fallbackTexture, fallbackStacks, fallbackDispelType
      local maxSlots = buff.btype == "HELPFUL" and 32 or 16

      if buff.id >= 1 and buff.id <= maxSlots then
        if buff.btype == "HELPFUL" and C.buffs.buffs == "1" then
          for i = 1, maxSlots do
            local tex, stacks, dtype = GetAuraInfoByIndex(i, "HELPFUL")
            if tex and i == buff.id then
              fallbackTexture, fallbackStacks, fallbackDispelType = tex, stacks, dtype
              break
            end
        ...
        elseif buff.btype == "HARMFUL" and C.buffs.debuffs == "1" then
          for i = 1, maxSlots do
            local tex, stacks, dtype = GetAuraInfoByIndex(i, "HARMFUL")
            if tex and i == buff.id then
              fallbackTexture, fallbackStacks, fallbackDispelType = tex, stacks, dtype
              break
            end
  ...
  local function CreateBuffButton(i, btype, weapon)
    ...
    buff.texture = buff:CreateTexture("BuffIcon" .. i, "ARTWORK")
    ...
    buff:SetScript("OnEnter", function()
      ...
      if this.mode == this.btype then
        if this.btype == "HELPFUL" and GameTooltip.SetUnitBuff then
          GameTooltip:SetUnitBuff("player", this.id)
        elseif this.btype == "HARMFUL" and GameTooltip.SetUnitDebuff then
          GameTooltip:SetUnitDebuff("player", this.id)
        elseif GameTooltip.SetPlayerBuff then
          GameTooltip:SetPlayerBuff(this.bid)
        end

        if IsShiftKeyDown() then
          local texture = GetAuraInfoByIndex(this.id, this.btype)
    ...
    buff:SetScript("OnClick", function()
      ...
      elseif CancelUnitBuff and this.btype == "HELPFUL" then
  ...
  local function GetNumBuffs()
    ...
    if pfUI.client >= 20000 then
      for i=1,32 do
        local texture = GetAuraInfoByIndex(i, "HELPFUL")
        if not texture then
          return i - 1 + offset
        end
      end
  ...
  if pfUI.client >= 20000 then
    ...
    pfUI.buff:RegisterEvent("PLAYER_ENTERING_WORLD")
  end
  ...
  pfUI.buff:SetScript("OnUpdate", function()
    ...
    for i = 1, 32 do
      ...
      if buff:IsShown() then
        ...
        if buff.mode == buff.btype then
          local _, count, _, _, _, auraTimeleft = GetAuraInfoByIndex(buff.id, buff.btype)
          timeleft = auraTimeleft or 0
          stacks = count or 0
    ...
    for i = 1, 16 do
      local buff = buttons[i]
      if buff:IsShown() then
        local _, stacks, _, _, _, timeleft = GetAuraInfoByIndex(buff.id, buff.btype)
