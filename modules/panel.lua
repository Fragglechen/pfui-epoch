pfUI:RegisterModule("panel", "vanilla:tbc:wotlk", function()
  ...
  pfUI.panel.central = CreateFrame("Frame", "pfCentralZoneFrame", UIParent)
  pfUI.panel.central:SetPoint("TOP", UIParent, "TOP", 0, -5)
  pfUI.panel.central:SetWidth(628)
  pfUI.panel.central:SetHeight(C.global.font_size * 1.6)
  pfUI.panel.central:SetFrameStrata("MEDIUM")
  UpdateMovable(pfUI.panel.central)

  local function CreateCentralSegment(parent, name, width, point, relative, relativePoint, xoff, yoff)
    local frame = CreateFrame("Frame", name, parent)
    frame:SetWidth(width)
    frame:SetHeight(parent:GetHeight())
    frame:SetPoint(point, relative, relativePoint, xoff, yoff)
  ...
  pfUI.panel.central.left = CreateCentralSegment(pfUI.panel.central, "pfCentralZoneFrameLeft", 120, "LEFT", pfUI.panel.central, "LEFT", 0, 0)
  pfUI.panel.central.leftcoord = CreateCentralSegment(pfUI.panel.central, "pfCentralZoneFrameLeftCoord", 56, "LEFT", pfUI.panel.central.left, "RIGHT", 4, 0)
  pfUI.panel.central.center = CreateCentralSegment(pfUI.panel.central, "pfCentralZoneFrameCenter", 264, "LEFT", pfUI.panel.central.leftcoord, "RIGHT", 4, 0)
  pfUI.panel.central.rightcoord = CreateCentralSegment(pfUI.panel.central, "pfCentralZoneFrameRightCoord", 56, "LEFT", pfUI.panel.central.center, "RIGHT", 4, 0)
  pfUI.panel.central.right = CreateCentralSegment(pfUI.panel.central, "pfCentralZoneFrameRight", 120, "LEFT", pfUI.panel.central.rightcoord, "RIGHT", 4, 0)

  pfUI.panel.central:SetScript("OnUpdate", function()
    ...
    this.left.text:SetText(string.format("%d%% %s", lowestPercent, ARMOR))

    local x, y = GetPlayerMapPosition("player")
    if x and y and (x > 0 or y > 0) then
      this.leftcoord.text:SetText(string.format("%.1f", x * 100))
      this.rightcoord.text:SetText(string.format("%.1f", y * 100))
    else
      this.leftcoord.text:SetText("--.-")
      this.rightcoord.text:SetText("--.-")
    end

    local zoneText = GetMinimapZoneText()
    if not zoneText or zoneText == "" then zoneText = GetSubZoneText() end
    if not zoneText or zoneText == "" then zoneText = GetRealZoneText() end
    this.center.text:SetText(zoneText or "")

    local timeText
    if C.global.twentyfour == "0" then
      timeText = date("%I:%M:%S %p")
    else
      timeText = date("%H:%M:%S")
    end
    this.right.text:SetText(timeText)
  end)

  -- MicroButtons
  if C.panel.micro.enable == "1" then
    pfUI.panel.microbutton = CreateFrame("Frame", "pfPanelMicroButton", UIParent)
    pfUI.panel.microbutton:SetPoint("TOPLEFT", pfUI.panel.minimap, "BOTTOMLEFT", 0, -2*default_border)
    ...
    for i=1,table.getn(MICRO_BUTTONS) do
      local anchor = lastButton or pfUI.panel.microbutton
      local button = _G[MICRO_BUTTONS[i]]
      if not button then
        -- skip unavailable client-specific buttons instead of aborting the whole microbar
      else
    ...
    if visibleButtons > 0 then
      ...
      pfUI.panel.microbutton:SetWidth(2 + visibleButtons * buttonWidth)
    end

    local function UpdateMicrobarVisibility()
      local show = true
      ...
      if show then
        pfUI.panel.microbutton:Show()
      else
        pfUI.panel.microbutton:Hide()
      end
    end

    pfUI.panel.microbutton.updater = CreateFrame("Frame", nil, pfUI.panel.microbutton)
    pfUI.panel.microbutton.updater:RegisterEvent("PLAYER_ENTERING_WORLD")
    pfUI.panel.microbutton.updater:RegisterEvent("UPDATE_BONUS_ACTIONBAR")
    pfUI.panel.microbutton.updater:RegisterEvent("ACTIONBAR_PAGE_CHANGED")
    pfUI.panel.microbutton.updater:RegisterEvent("PLAYER_CONTROL_GAINED")
    pfUI.panel.microbutton.updater:RegisterEvent("PLAYER_CONTROL_LOST")
    pfUI.panel.microbutton.updater:SetScript("OnEvent", UpdateMicrobarVisibility)
    UpdateMicrobarVisibility()
  end
