pfUI:RegisterModule("bags", "vanilla:tbc:wotlk", function ()
  local rawborder, default_border = GetBorderSize("bags")
  local BlizzardOpenAllBags = _G.OpenAllBags
  local BlizzardCloseAllBags = _G.CloseAllBags
  local BlizzardToggleBackpack = _G.ToggleBackpack
  local BlizzardOpenBackpack = _G.OpenBackpack
  ...
  local function ClosePfUIBags()
    if not pfUI.bag or not pfUI.bag.right then return end
    pfUI.bag.right:Hide()
  end

  local function TogglePfUIBackpack()
    if not pfUI.bag or not pfUI.bag.right then return end
    if pfUI.bag.right:IsShown() then
      pfUI.bag.right:Hide()
    else
  ...
  local function OpenPfUIBackpack()
    if not pfUI.bag or not pfUI.bag.right then return end
    if ( pfUI.bag.right:IsShown() ) then
  ...
  -- Always prefer pfUI bags for user-triggered bag toggles (including combat).
  -- Any protected edge-cases are handled separately (e.g. temporary spell targeting fallback).
  ...
  local function ShowBagItemTooltip(button)
    ...
    local bag = button.bag or (button.GetParent and button:GetParent() and button:GetParent():GetID())
    local slot = button.slot or (button.GetID and button:GetID())
    if bag == nil or slot == nil then return end

    pfUI.bag.hoveredTooltipButton = button
    GameTooltip:SetOwner(UIParent, "ANCHOR_CURSOR")
    GameTooltip:ClearLines()

    -- Bank main slots (bag -1) require inventory-slot based tooltips on this client.
    if bag == -1 and type(_G.BankButtonIDToInvSlotID) == "function" then
  ...
  local function HideBagItemTooltip(button)
    if not button then button = this end
    if not button then return end

    local parent = button.GetParent and button:GetParent() or nil
    local focus = GetMouseFocus and GetMouseFocus() or nil
    if focus == parent or (parent and IsCursorOverFrame(parent)) then
      return
    end

    if pfUI.bag and pfUI.bag.hoveredTooltipButton == button then
      pfUI.bag.hoveredTooltipButton = nil
    end
  ...
  pfUI.bag:SetScript("OnUpdate", function()
    -- Delay bag updates slightly so protected bag-slot clicks (e.g. disenchant)
    -- can finish before we touch the container buttons again.
    if ( this.tick or 1) > GetTime() then return else this.tick = GetTime() + .1 end

    if this.hoveredTooltipButton then
      local button = this.hoveredTooltipButton
      local parent = button.GetParent and button:GetParent() or nil
      local stillHovered = IsCursorOverFrame(button) or (parent and IsCursorOverFrame(parent))

      if stillHovered and button.hasItem then
        local bag = button.bag or (parent and parent.GetID and parent:GetID())
        local slot = button.slot or (button.GetID and button:GetID())
        ...
        this.hoveredTooltipButton = nil
        GameTooltip:Hide()
      end
    end

    if this.useBlizzardTargeting and SpellIsTargeting and not SpellIsTargeting() then
      pfUI.bag:LeaveBlizzardBagTargeting()
    end

    if this.delay.EnterBlizzardTargeting then
      this.delay.EnterBlizzardTargeting = nil
      if SpellIsTargeting and SpellIsTargeting() then
        pfUI.bag:EnterBlizzardBagTargeting()
      end
    end
  ...
  function pfUI.bag:CheckFullUpdate()
    ...
    if maxslots ~= pfUI.bag.maxslots then
      for bag = -2,11 do
        if pfUI.bags[bag] and pfUI.bags[bag].slots then
          for slot, f in ipairs(pfUI.bags[bag].slots) do
            local bagsize = GetContainerNumSlots(bag)
            if bag == -2 and pfUI.bag.showKeyring == true then bagsize = GetKeyRingSize() end

            if slot > bagsize and pfUI.bags[bag].slots[slot].frame then
  ...
  function pfUI.bag:EnterBlizzardBagTargeting()
    if not pfUI.bag or not pfUI.bag.right then return end
    if not pfUI.bag.right:IsShown() then return end
    if pfUI.bag.useBlizzardTargeting then return end

    pfUI.bag.wasShownBeforeTargeting = pfUI.bag.right:IsShown() and 1 or nil
    pfUI.bag.useBlizzardTargeting = 1
    pfUI.bag.right:Hide()

    if BlizzardOpenAllBags then
      BlizzardOpenAllBags()
    elseif BlizzardOpenBackpack then
      BlizzardOpenBackpack()
    end
  end

  function pfUI.bag:LeaveBlizzardBagTargeting()
    if not pfUI.bag or not pfUI.bag.useBlizzardTargeting then return end

    if BlizzardCloseAllBags then
      BlizzardCloseAllBags()
    end

    if pfUI.bag.wasShownBeforeTargeting and pfUI.bag.right then
      pfUI.bag.right:Show()
    end

    pfUI.bag.wasShownBeforeTargeting = nil
    pfUI.bag.useBlizzardTargeting = nil
  end

  function pfUI.bag:QueueBlizzardBagTargeting()
    if not pfUI.bag then return end
    pfUI.bag.delay.EnterBlizzardTargeting = true
  end
  ...
  function pfUI.bag:UpdateCooldowns()
    ...
    for bag=-2, 11 do
      ...
      for slot=1, bagsize do
        frame = pfUI.bags[bag] and pfUI.bags[bag].slots and pfUI.bags[bag].slots[slot] and pfUI.bags[bag].slots[slot].frame
  ...
  function pfUI.bag:UpdateSlot(bag, slot)
    ...
    if not pfUI.bags[bag].slots[slot] then
      ...
      pfUI.bags[bag].slots[slot].frame.bag = bag
      pfUI.bags[bag].slots[slot].frame.slot = slot
      pfUI.bags[bag].slots[slot].frame.pfOriginalOnEnter = pfUI.bags[bag].slots[slot].frame:GetScript("OnEnter")
      pfUI.bags[bag].slots[slot].frame.pfOriginalOnLeave = pfUI.bags[bag].slots[slot].frame:GetScript("OnLeave")
      ...
      if not pfUI.bags[bag].slots[slot].frame.pfTooltipDebugWrapped then
        pfUI.bags[bag].slots[slot].frame:SetScript("OnEnter", function(self)
          ShowBagItemTooltip(self)
        end)

        pfUI.bags[bag].slots[slot].frame:SetScript("OnLeave", function(self)
          HideBagItemTooltip(self)
        end)

        pfUI.bags[bag].slots[slot].frame.pfTooltipDebugWrapped = true
      end
    end
    ...
    pfUI.bags[bag].slots[slot].frame.bag = bag
    pfUI.bags[bag].slots[slot].frame.slot = slot
    pfUI.bags[bag].slots[slot].frame:EnableMouse(1)
  ...
  function pfUI.bag:CreateBagSlots(frame)
    ...
    if frame == pfUI.bag.left then
      if GetNumBankSlots() < NUM_BANKBAGSLOTS then
        ...
        if frame.bagslots.buy.SetText then
          frame.bagslots.buy:SetText("+")
        end
        if frame.bagslots.buy.SetTextColor then
          frame.bagslots.buy:SetTextColor(.5,.5,1,1)
        end
        if frame.bagslots.buy.SetFont then
      ...
      for texture, widget in pairs(knownInventorySpellTextures) do
        if spellTexture and texture and strfind(spellTexture, texture) and pfUI.bag.right[widget.frame] then
          ...
          if not (InCombatLockdown and InCombatLockdown()) then
            ...
            pfUI.bag.right[widget.frame]:SetAttribute("type", "spell")
            pfUI.bag.right[widget.frame]:SetAttribute("spell", spellName)
          end
        end
      end
    end
  ...
  function pfUI.bag:CreateAdditions(frame)
    ...
    if frame == pfUI.bag.right then
      ...
      if not frame.bags then
        ...
        if frame.bags.SetTextColor then
          frame.bags:SetTextColor(1,1,.25,1)
        end
        if frame.bags.SetFont then
        ...
        frame.bags:SetScript("OnClick", function()
          if not pfUI.bag.right or not pfUI.bag.right.bagslots then return end
      ...
      if not frame.open then
        ...
        if frame.open.SetTextColor then
          frame.open:SetTextColor(1,1,.25,1)
        end
        if frame.open.SetFont then
      ...
      if not frame.disenchant then
        ...
        if frame.disenchant.SetTextColor then
          frame.disenchant:SetTextColor(1,1,.25,1)
        end
        if frame.disenchant.SetFont then
      ...
      if not frame.picklock then
        ...
        if frame.picklock.SetTextColor then
          frame.picklock:SetTextColor(1,1,.25,1)
        end
        if frame.picklock.SetFont then
      ...
      if not frame.keys then
        ...
        if frame.keys.SetTextColor then
          frame.keys:SetTextColor(1,1,.25,1)
        end
        if frame.keys.SetFont then
      ...
      if not frame.bags then
        ...
        frame.bags:SetScript("OnClick", function()
          if not pfUI.bag.left or not pfUI.bag.left.bagslots then return end
