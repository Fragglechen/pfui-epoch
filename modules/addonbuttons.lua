pfUI:RegisterModule("addonbuttons", "vanilla:tbc:wotlk", function ()
  ...
  local function NormalizeButton(frame)
  ...
  local function MoveButton(index, frame)
    if not frame then return end
    local top_frame, row_index, offsetX, offsetY, final_scale, button_scale, nudge_x, nudge_y
    ...
    if frame:GetName() == "pfMinimapPvPQueue" then
      button_scale = .88
      nudge_x = 3
      nudge_y = 3
    elseif frame:GetName() == "AtlasButton" or (top_frame and top_frame:GetName() == "AtlasButtonFrame") then
      button_scale = .78
      nudge_x = -1
      nudge_y = -5
    end
    ...
    if top_frame ~= pfUI.addonbuttons then
      top_frame:SetScale(final_scale * button_scale)
    ...
    if C.abuttons.position == "bottom" or C.abuttons.position == "top" then
      ...
      frame:SetPoint("CENTER", pfUI.addonbuttons, "TOPLEFT", (offsetX + nudge_x)/final_scale, (offsetY + nudge_y)/final_scale)
      if top_frame ~= pfUI.addonbuttons then
        top_frame:SetPoint("CENTER", pfUI.addonbuttons, "TOPLEFT", (offsetX + nudge_x)/final_scale, (offsetY + nudge_y)/final_scale)
      end
    else
      ...
      frame:SetPoint("CENTER", pfUI.addonbuttons, "TOPRIGHT", (offsetX + nudge_x)/final_scale, (offsetY + nudge_y)/final_scale)
      if top_frame ~= pfUI.addonbuttons then
        top_frame:SetPoint("CENTER", pfUI.addonbuttons, "TOPRIGHT", (offsetX + nudge_x)/final_scale, (offsetY + nudge_y)/final_scale)
      end
    end
  end
