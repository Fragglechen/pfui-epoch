pfUI:RegisterModule("tracking", "vanilla:tbc:wotlk", function ()
  ...
  for _, frame in pairs(trackingFrames) do
    if frame then
  ...
  pfUI.tracking:SetScript("OnClick", function()
    ...
    if arg1 == "LeftButton" and state.texture then
      if type(_G.CancelTrackingBuff) == "function" then
        CancelTrackingBuff()
      elseif type(_G.GetNumTrackingTypes) == "function" and type(_G.GetTrackingInfo) == "function" and type(_G.SetTracking) == "function" then
  ...
  pfUI.tracking:SetScript("OnEnter", function()
    ...
    if state.texture then
      if GameTooltip.SetTrackingSpell then
        GameTooltip:SetTrackingSpell()
      else
        GameTooltip:SetText(T["Tracking"])
      end
    else
