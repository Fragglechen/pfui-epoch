...
local function OnDragStop()
  ...
  if this.SetUserPlaced then
    this:SetUserPlaced(true)
  end
  if pfUI and pfUI.api and pfUI.api.SaveMovable and this.GetName and this:GetName() then
    pfUI.api.SaveMovable(this)
  end
end

local function HookMovableOnShow(frame)
  if not frame or frame.pfUIOnShowHooked then return end
  frame.pfUIOnShowHooked = true
  frame:HookScript("OnShow", function(self)
    if pfUI and pfUI.api and pfUI.api.LoadMovable then
      pfUI.api.LoadMovable(self)
    end
  end)
end
    ...
    scan:SetScript("OnEvent", function()
      if arg1 == addon then
        ...
        frame:RegisterForDrag("LeftButton")
          ...
          HookMovableOnShow(frame)

          if pfUI and pfUI.api and pfUI.api.LoadMovable then
            pfUI.api.LoadMovable(frame, true)
          end
...
function pfUI.api.CreateBackdrop(f, inset, legacy, transp, backdropSetting)
  ...
  local appearance = pfUI_config and pfUI_config.appearance or {}
  local bordercfg = appearance.border or {}
  local blizz = bordercfg.force_blizz == "1" and true or nil
  ...
  br, bg, bb, ba = pfUI.api.GetStringColor(bordercfg.background or "0,0,0,1")
  er, eg, eb, ea = pfUI.api.GetStringColor(bordercfg.color or ".4,.4,.4,1")
