...
local function GetAuraInfoByIndex(unitstr, index, filter)
  index = tonumber(index)
  if not unitstr or not index or index < 1 then return nil end

  local helpful = filter == "HELPFUL"

  if pfUI.client > 11200 then
    local a, b, c, d, e, f = helpful and UnitBuff(unitstr, index) or UnitDebuff(unitstr, index)
    ...
    local dtype = helpful and nil or ((e == "Magic" or e == "Poison" or e == "Curse" or e == "Disease") and e
      or (d == "Magic" or d == "Poison" or d == "Curse" or d == "Disease") and d or nil)
    local duration = type(e) == "number" and e or type(f) == "number" and f or nil
    local timeleft = type(f) == "number" and f or nil
    return texture, count, dtype, duration, timeleft
  end

  if unitstr == "player" then
    local bid = GetPlayerBuff(PLAYER_BUFF_START_ID + index, filter)
    if not bid or bid < 0 then return nil end
    return GetPlayerBuffTexture(bid), GetPlayerBuffApplications(bid, filter), GetPlayerBuffDispelType(bid), nil, GetPlayerBuffTimeLeft(bid, filter)
  end

  return helpful and UnitBuff(unitstr, index) or UnitDebuff(unitstr, index)
end

local maxdurations = {}
local function BuffOnUpdate()
  if ( this.tick or 1) > GetTime() then return else this.tick = GetTime() + .2 end
  local texture, _, _, _, timeleft = GetAuraInfoByIndex("player", this.id, "HELPFUL")
  local start = 0
  ...
  CooldownFrame_SetTimer(this.cd, start, maxdurations[texture] or 0, timeleft and timeleft > 0 and 1 or 0)
end

local function TargetBuffOnUpdate()
  local _, _, _, duration, timeleft = GetAuraInfoByIndex("target", this.id, "HELPFUL")
...
local function BuffOnEnter()
  ...
  if IsShiftKeyDown() then
    local texture = parent.label == "player" and GetAuraInfoByIndex("player", this.id, "HELPFUL") or GetAuraInfoByIndex(parent.label .. parent.id, this.id, "HELPFUL")
...
local function BuffOnClick()
  if this:GetParent().label == "player" then
    if CancelUnitBuff then
      CancelUnitBuff("player", tonumber(this.id))
...
local function DebuffOnUpdate()
  if ( this.tick or 1) > GetTime() then return else this.tick = GetTime() + .2 end
  local texture, _, _, _, timeleft = GetAuraInfoByIndex("player", this.id, "HARMFUL")
  local start = 0
...
function pfUI.uf:RefreshUnit(unit, component)
  ...
  if unit.buffs and ( component == "all" or component == "aura" ) then
    ...
    for i=1, unit.config.bufflimit do
      ...
      if unit.label == "player" then
        texture, stacks = GetAuraInfoByIndex(unitstr, i, "HELPFUL")
      else
  ...
  if unit.debuffs and ( component == "all" or component == "aura" ) then
    ...
    for i=1, unit.config.debufflimit do
      ...
      if unit.label == "player" then
        texture, stacks, dtype = GetAuraInfoByIndex(unitstr, i, "HARMFUL")
      else
      ...
      if texture then
        ...
        if unit:GetName() == "pfPlayer" then
          local _, _, _, _, timeleft = GetAuraInfoByIndex(unitstr, unit.debuffs[i].id, "HARMFUL")
