...
pfUI.addonskinner:RegisterSkin("SpecialTalentUI", function()
  ...
  local StripTextures, CreateBackdrop, SkinCloseButton, SkinButton, SkinArrowButton, 
    SkinCheckbox, SkinScrollbar, SetAllPointsOffset, SkinDropDown, SkinTab, HookScript = 
  penv.StripTextures, penv.CreateBackdrop, penv.SkinCloseButton, penv.SkinButton, penv.SkinArrowButton, 
  penv.SkinCheckbox, penv.SkinScrollbar, penv.SetAllPointsOffset, penv.SkinDropDown, penv.SkinTab, penv.HookScript
  ...
  local function FindFontStringByText(frame, pattern)
    ...
    if frame.GetChildren then
      for _, child in pairs({ frame:GetChildren() }) do
        local result = FindFontStringByText(child, pattern)
  ...
  local function FindButtonByText(frame, text)
    ...
    if frame.GetObjectType and frame:GetObjectType() == "Button" and frame.GetText then
      local value = frame:GetText()
      if value == text then return frame end
    end

    if frame.GetChildren then
      for _, child in pairs({ frame:GetChildren() }) do
        local result = FindButtonByText(child, text)
        if result then return result end
      end
    end
  end

  local function EnsureFooterFrame(name, parent, alpha)
    local frame = getglobal(name)
    if not frame then
      frame = CreateFrame("Frame", name, parent)
      CreateBackdrop(frame, nil, nil, alpha or .35)
    end
    frame:SetFrameStrata(parent:GetFrameStrata())
    frame:SetFrameLevel(parent:GetFrameLevel() + 80)
    if frame.backdrop then
      frame.backdrop:SetFrameStrata(parent:GetFrameStrata())
      frame.backdrop:SetFrameLevel(frame:GetFrameLevel())
      frame.backdrop:SetBackdropColor(0, 0, 0, .88)
    end
    frame:Show()
    return frame
  end

  local function LayoutSpecialTalentFooter()
    if not SpecialTalentFrame then return end

    local footer = EnsureFooterFrame("pfSpecialTalentFooter", SpecialTalentFrame, .45)
    footer:ClearAllPoints()
    footer:SetPoint("BOTTOMLEFT", SpecialTalentFrame.backdrop or SpecialTalentFrame, "BOTTOMLEFT", 18, 58)
    footer:SetPoint("BOTTOMRIGHT", SpecialTalentFrame.backdrop or SpecialTalentFrame, "BOTTOMRIGHT", -18, 88)

    local spent = FindFontStringByText(SpecialTalentFrame, " Talents:")
    if spent then
      spent:ClearAllPoints()
      spent:SetPoint("TOPLEFT", footer, "TOPLEFT", 12, -9)
      spent:SetJustifyH("LEFT")
      spent:SetTextColor(1, .82, 0)
      spent:SetDrawLayer("OVERLAY")
      if spent.SetParent then spent:SetParent(footer) end
    end

    local unspent = FindFontStringByText(SpecialTalentFrame, "Unspent Talents")
    if unspent then
      unspent:ClearAllPoints()
      unspent:SetPoint("TOPRIGHT", footer, "TOPRIGHT", -12, -9)
      unspent:SetJustifyH("RIGHT")
      unspent:SetTextColor(1, .82, 0)
      unspent:SetDrawLayer("OVERLAY")
      if unspent.SetParent then unspent:SetParent(footer) end
    end

    local learn = FindButtonByText(SpecialTalentFrame, "Learn")
    local reset = FindButtonByText(SpecialTalentFrame, "Reset")

    if learn then
      SkinButton(learn)
      learn:ClearAllPoints()
      learn:SetPoint("BOTTOMRIGHT", footer, "BOTTOM", -6, 7)
      learn:SetWidth(130)
      learn:SetHeight(26)
      learn:SetFrameLevel(footer:GetFrameLevel() + 2)
      learn:SetFrameStrata(footer:GetFrameStrata())
    end

    if reset then
      SkinButton(reset)
      reset:ClearAllPoints()
      reset:SetPoint("BOTTOMLEFT", footer, "BOTTOM", 6, 7)
      reset:SetWidth(130)
      reset:SetHeight(26)
      reset:SetFrameLevel(footer:GetFrameLevel() + 2)
      reset:SetFrameStrata(footer:GetFrameStrata())
    end
  end

  local function LayoutSpecialTalentTabs()
    if not SpecialTalentFrame then return end
    ...
    for i = 1, 5 do
      ...
      if tab and tab:IsShown() then
        if SkinTab then SkinTab(tab) else SkinButton(tab) end
        tab:SetScale(1)
        tab:SetWidth(maxWidth)
        if getglobal(tab:GetName().."Middle") then
          getglobal(tab:GetName().."Middle"):SetWidth(maxWidth - 20)
        end
        tab:ClearAllPoints()
        if not last then
          tab:SetPoint("TOPLEFT", SpecialTalentFrame.backdrop or SpecialTalentFrame, "BOTTOMLEFT", 12, -2)
        else
          tab:SetPoint("LEFT", last, "RIGHT", 3, 0)
        end
        last = tab
      end
    end
  ...
  local function LayoutSpecialTalentSkin()
    SkinAllTalentButtons()
    LayoutSpecialTalentFooter()
    LayoutSpecialTalentTabs()
  end
  ...
  if SpecialTalentFrame_Update then
    ...
    SpecialTalentFrame_Update = function()
      orig_update()
      LayoutSpecialTalentSkin()
    end
  end
  
  -- Skin on show
  if SpecialTalentFrame then
    HookScript(SpecialTalentFrame, "OnShow", LayoutSpecialTalentSkin)
  end
end)
