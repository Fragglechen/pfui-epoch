...
-->>--  AtlasFrame
  ...
  if pfUI.addonbuttons and AtlasButton then
    local function AddAtlasButton()
      pfUI_cache["abuttons"] = pfUI_cache["abuttons"] or {}
      pfUI_cache["abuttons"]["add"] = pfUI_cache["abuttons"]["add"] or {}

      for _, button_name in ipairs(pfUI_cache["abuttons"]["add"]) do
        if button_name == "AtlasButton" then
          return
        end
      end

      table.insert(pfUI_cache["abuttons"]["add"], "AtlasButton")
    end

    AddAtlasButton()
    pfUI.addonbuttons:ProcessButtons()

    if AtlasButton_UpdatePosition then
      hooksecurefunc("AtlasButton_UpdatePosition", function()
